...
CASE
	WHEN standard_label IN ('newspaper', 'daily-newspaper', 'periodical', 'magazine', 'online-newspaper', 'open-access-publisher', 'news-magazine', 'news-website', 'weekly-magazine', 'weekly-newspaper', 'tabloid', 'newspaper-of-record')
	THEN 'press'
	WHEN standard_label IN ('book-publisher', 'publishing-house')
	THEN 'publishing'  
	WHEN standard_label IN ('television-station', 'radio-station', 'broadcaster', 'public-broadcaster', 'television-network', 'specialty-channel', 'media-company', 'television-channel', 'radio-network', 'public-broadcasting', 'generalist-television-channel', 'broadcast-network')
	THEN 'television and radio'
	WHEN standard_label IN ('news-agency', 'photo-agency', 'production-company', 'television-production-company', 'photographic-studio', 'film-studio', 'film-production-company')
	THEN 'agences and production'
	WHEN standard_label IN ('%univers%', '%college%', '%school%', 'higher-education-institution', 'educational-institution', 'academy', 'institute-of-technology')
	THEN 'academic and education'
	WHEN standard_label IN ('%ministry%', '%military%', 'government-agency', 'armed-forces', 'intelligence-agency', 'navy', 'army', 'government-organization', 'state-media')
	THEN 'government and military'
	WHEN standard_label IN ('business', 'organization', 'enterprise', 'nonprofit-organization', 'state-owned-enterprise', 'website', 'public-company', 'privately-held-company', 'corporation', 'company', 'web-portal')
	THEN 'corporate and general'
	-- see beelow
	ELSE NULL
...
CASE
	WHEN standard_label IN ('newspaper', 'daily-newspaper', 'periodical', 'magazine', 'online-newspaper', 'open-access-publisher', 'news-magazine', 'news-website', 'weekly-magazine', 'weekly-newspaper', 'tabloid', 'newspaper-of-record', 'government-gazette', 'catholic-newspaper', 'communist-newspaper', 'party-newspaper', 'student-newspaper', 'tabloid-newspaper', 'news-aggregation-website', 'online-mass-media', 'hate-media', 'serial', 'banned-work', 'mass-media')
		OR standard_label LIKE '%journal%'
	    OR standard_label LIKE '%magazine%'
	    OR standard_label = 'women''s-magazine'
	THEN 'press'
	WHEN standard_label IN ('book-publisher', 'publishing-house')
	THEN 'publishing'  
	WHEN standard_label IN ('news-program', 'united-states-cable-news', 'youtube-channel', 'television-station', 'radio-station', 'broadcaster', 'public-broadcaster', 'television-network', 'specialty-channel', 'media-company', 'television-channel', 'radio-network', 'public-broadcasting', 'generalist-television-channel', 'broadcast-network')
		OR standard_label LIKE '%television%'
	    OR standard_label LIKE '%radio%'
	    OR standard_label LIKE '%broadcast%'
	THEN 'television and radio'
	WHEN standard_label IN ('major-label', 'video-game-publisher', 'news-agency', 'photo-agency', 'production-company', 'television-production-company', 'photographic-studio', 'film-studio', 'film-production-company')
		OR standard_label LIKE '%production%'
    	OR standard_label LIKE '%studio%'
	THEN 'agences and production'
	WHEN standard_label IN ('art-academy', 'czech-research-institution', 'institute-of-the-russian-academy-of-science', 'kantonsschule', 'pla-academy', 'polytechnic', 'higher-education-institution', 'educational-institution', 'academy', 'institute-of-technology')
		OR standard_label LIKE '%univers%'
		OR standard_label LIKE '%college%' 
		OR standard_label LIKE '%school%' 
    	OR standard_label LIKE '%academic%'
    	OR standard_label LIKE '%educational%'
	THEN 'academic and education'
	WHEN standard_label IN ('engineer-unit', 'alliance', 'command', 'command-of-the-bundeswehr', 'division', 'executive-branch', 'foreign-agent', 'political-police', 'province-of-china', 'secret-police', 'united-nations-depository-library', 'government-agency', 'armed-forces', 'intelligence-agency', 'navy', 'army', 'government-organization', 'state-media')
		OR standard_label LIKE '%ministry%' 
    	OR standard_label LIKE '%military%' 
    	OR standard_label LIKE '%government%'
    	OR standard_label LIKE '%agency%'
	THEN 'government and military'
	WHEN standard_label IN ('cooperative', 'crown-corporation', 'public-enterprise', 'conglomerate', 'institution', 'local-internet-registry', 'national-red-cross-and-red-crescent-society', 'nonprofit-corporation', 'ottoman-association', 'regional-standards-organisation', 'statutory-corporation', 'unitary-enterprise', 'voluntary-association', 'business', 'organization', 'enterprise', 'nonprofit-organization', 'state-owned-enterprise', 'website', 'public-company', 'privately-held-company', 'corporation', 'company', 'web-portal')
		OR standard_label LIKE '%organization%'
    	OR standard_label LIKE '%company%'
	THEN 'corporate and general'
	ELSE standard_label
END;
...
-- More than 70% have between one and four jobs
...
SET employer_2 = (
    ...
    AND ioc.coded_class NOT IN ('press')   --pf.employer_1
    ORDER BY stats.global_count DESC, ioc.coded_class ASC
    LIMIT 1
)
WHERE 
...
/*
 * Add finally press where no other employer available,
 * or as a prefix if press and another employer
 */
select 
  CASE 
  	WHEN employer_1 = 'press' AND employer_2 is null
  	THEN 'press'
  	WHEN employer_1 = 'press' AND employer_2 is not null
  	THEN 'press' || employer_2
	ELSE employer_2
  END coded_employer,
...
UPDATE person_features 
SET coded_employer = CASE 
    WHEN employer_1 = 'press' AND employer_2 IS NULL
        THEN 'press'
    WHEN employer_1 = 'press' AND employer_2 IS NOT NULL
        THEN 'press / ' || employer_2
    ELSE COALESCE(employer_1, employer_2)
END 
WHERE employer_1 IS NOT NULL 
   OR employer_2 IS NOT NULL;
...
SELECT pk_person_features, person_uri, occupation_main, occupation_sec2, coded_employer
from person_features 
WHERE occupation_sec1 != 'NA'
AND occupation_main != 'NA'
AND coded_employer is not NULL;
